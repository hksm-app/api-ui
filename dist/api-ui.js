...
(function (global) {
  ...
  function _resolveDefaultFavicon() {
    ...
    for (let i = 0; i < scripts.length; i++) {
      ...
      if (src && src.includes('api-ui.js')) {
        return src.replace('api-ui.js', 'favicon.ico');
      }
    }
    return null;
  }
...
<div id="hau-body">
  ...
  // ── TOKEN STORAGE ───────────────────────────────────────────────────────────
  function _getToken()  { return localStorage.getItem(cfg.tokenKey) || ''; }
  function _saveToken() {
    ...
    _updateAuthUI();
    const b = _el('hau-try-bearer'); if (b) b.value = val;
    _closeAuthDropdown();
  }
  function _clearToken() {
    localStorage.removeItem(cfg.tokenKey);
    _el('hau-auth-token-input').value = '';
    const b = _el('hau-try-bearer'); if (b) b.value = '';
    _updateAuthUI();
  }
  ...
  function _updateAuthUI() {
    ...
    if (token) {
      ...
      if (bar) { bar.className = 'hau-auth-status-bar ok'; bar.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="20 6 9 17 4 12"/></svg><span>Token saved. All Try It requests will be authorized.</span>'; }
      if (ls)  { ls.className = 'authed'; }
      ...
      if (bar) { bar.className = 'hau-auth-status-bar empty'; bar.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg><span>No token set.</span>'; }
      if (ls)  { ls.className = 'noauth'; }
  ...
  function _toggleAuthDropdown() {
    ...
    if (isOpen) { _el('hau-auth-token-input').value = _getToken(); setTimeout(() => _el('hau-auth-token-input').focus(), 50); }
  }
  function _closeAuthDropdown() { _el('hau-auth-dropdown').classList.remove('open'); }
  ...
  async function _loadSpec() {
    ...
    const b = _el('hau-try-bearer'); if (b && stored) b.value = stored;
  }

  function _buildStats() {
    const paths = spec.paths || {};
    let total = 0, gets = 0, posts = 0; const tagSet = new Set();
    Object.entries(paths).forEach(([, methods]) => {
      Object.entries(methods).forEach(([method, op]) => {
        if (!['get','post','put','patch','delete'].includes(method)) return;
        total++; if (method === 'get') gets++; if (method === 'post') posts++;
  ...
  function _buildSidebar() {
    const paths = spec.paths || {}; const tags = {};
    Object.entries(paths).forEach(([path, methods]) => {
      Object.entries(methods).forEach(([method, op]) => {
        if (!['get','post','put','patch','delete'].includes(method)) return;
        const tl = (op.tags && op.tags.length) ? op.tags : ['Default'];
        tl.forEach(tag => { if (!tags[tag]) tags[tag] = []; tags[tag].push({ method: method.toUpperCase(), path, op }); });
      });
    });
    const nav = _el('hau-sb-inner'); nav.innerHTML = '';
    Object.entries(tags).sort((a, b) => a[0].localeCompare(b[0])).forEach(([tag, eps]) => {
      const g = document.createElement('div'); g.className = 'hau-sb-group';
      const hd = document.createElement('div'); hd.className = 'hau-sb-group-hd';
      ...
      const list = document.createElement('div'); list.className = 'hau-sb-eps';
      eps.forEach(ep => {
        const btn = document.createElement('button'); btn.className = 'hau-sb-ep';
        btn.dataset.method = ep.method; btn.dataset.path = ep.path;
      ...
      hd.addEventListener('click', () => { const o = list.classList.toggle('open'); hd.classList.toggle('open', o); });
      g.appendChild(hd); g.appendChild(list); nav.appendChild(g);
    });
  }
  ...
  function _openOp(method, path, op, btn) {
    ...
    badge.textContent = method; badge.className = `hau-ep-method-badge hau-mb hau-mb-${method}`;
    badge.style.fontSize = '13px'; badge.style.padding = '4px 12px';
    ...
    _el('hau-doc').scrollTo({ top: 0, behavior: 'smooth' });
  }
  ...
  function _renderEpBody(method, path, op) {
    const body = _el('hau-ep-body'); let html = '';
    ...
    if (allParams.length) {
      ...
      allParams.forEach(p => {
        const s = p.schema || {}; const type = s.type || 'string';
    ...
    if (op.responses) {
      ...
      Object.entries(op.responses).forEach(([code, resp]) => {
        ...
        const ct2 = resp.content || {}; let hasContent = false;
        Object.entries(ct2).forEach(([, media]) => { hasContent = true; if (media.schema) html += _codeBlock(JSON.stringify(_buildExample(media.schema), null, 2)); });
  ...
  function _renderTryPanel(method, path, op) {
    ...
    const b = _el('hau-try-bearer'); if (b) b.value = _getToken();
    const fields = _el('hau-try-fields'); fields.innerHTML = '';
    const pathParams   = (op.parameters || []).filter(p => p.in === 'path');
    const queryParams  = (op.parameters || []).filter(p => p.in === 'query');
    const headerParams = (op.parameters || []).filter(p => p.in === 'header');
    [['Path Parameters', pathParams, 'path', true], ['Query Parameters', queryParams, 'query', false], ['Header Parameters', headerParams, 'header', false]].forEach(([title, params, inType, isPath]) => {
      if (!params.length) return;
      let h = `<div class="hau-try-section"><div class="hau-try-section-title">${title}</div>`;
      params.forEach(p => { h += `<div class="hau-try-field"><div class="hau-try-label">${_esc(p.name)}${p.required ? '<span class="hau-req-star">*</span>' : ''} <span style="color:var(--hau-t3);font-weight:400;font-size:10px;">${inType}</span></div><input class="hau-try-input${isPath ? ' is-path-param' : ''}" data-in="${inType}" data-name="${_esc(p.name)}" placeholder="${_esc(p.description || p.name)}" /></div>`; });
      h += '</div>'; fields.innerHTML += h;
    });
    if (op.requestBody) {
      const ct = op.requestBody.content || {}; const mime = Object.keys(ct)[0] || 'application/json';
  ...
  async function _executeRequest(method, path, op) {
    ...
    let url = path; const queryParts = []; const extraHeaders = {};
    ...
    if (bodyEl && bodyEl.value.trim()) { fetchOpts.headers['Content-Type'] = bodyEl.dataset.mime || 'application/json'; fetchOpts.body = bodyEl.value.trim(); }
    const t0 = Date.now();
    try {
      const resp = await fetch(url, fetchOpts); const elapsed = Date.now() - t0;
      const text = await resp.text(); let pretty = text;
      ...
      const codeEl = _el('hau-try-resp-code'); codeEl.textContent = 'Error'; codeEl.style.color = 'var(--hau-del-c)';
  ...
  function _initResize() {
    const handle = _el('hau-try-response-resize'); const panel = _el('hau-try-response');
    if (!handle || !panel) return;
    const saved = localStorage.getItem('hau_resp_h'); if (saved) panel.style.height = saved + 'px';
    let startY = 0, startH = 0, dragging = false;
    handle.addEventListener('mousedown', e => { dragging = true; startY = e.clientY; startH = panel.getBoundingClientRect().height; handle.classList.add('dragging'); document.body.style.userSelect = 'none'; document.body.style.cursor = 'ns-resize'; e.preventDefault(); });
    document.addEventListener('mousemove', e => { if (!dragging) return; const d = startY - e.clientY; panel.style.height = Math.max(56, Math.min(startH + d, window.innerHeight * .8)) + 'px'; });
    document.addEventListener('mouseup', () => { if (!dragging) return; dragging = false; handle.classList.remove('dragging'); document.body.style.userSelect = ''; document.body.style.cursor = ''; localStorage.setItem('hau_resp_h', parseInt(panel.style.height)); });
  }

  // ── EVENTS ──────────────────────────────────────────────────────────────────
  function _bindEvents() {
    document.addEventListener('click', e => {
      const wrap = _el('hau-auth-wrap'); if (wrap && !wrap.contains(e.target)) _closeAuthDropdown();
    });
    document.addEventListener('keydown', e => { if (e.key === 'Escape') _closeAuthDropdown(); });

    // Logo → home (delegated since DOM may not exist yet at bind time)
    document.addEventListener('click', e => {
      const logo = e.target.closest('#hau-logo-link'); if (logo) { e.preventDefault(); _goHome(); }
    });
    ...
    // Tab switching
    document.addEventListener('click', e => {
      const tab = e.target.closest('.hau-try-tab'); if (!tab) return;
    ...
    document.addEventListener('input', e => {
      ...
      document.querySelectorAll('.hau-sb-group').forEach(g => {
        const eps = g.querySelectorAll('.hau-sb-ep'); let any = false;
        eps.forEach(ep => { const show = !q || ep.textContent.toLowerCase().includes(q); ep.classList.toggle('hau-sh', !show); if (show) any = true; });
        g.style.display = any ? '' : 'none';
        if (q && any) { g.querySelector('.hau-sb-eps').classList.add('open'); g.querySelector('.hau-sb-group-hd').classList.add('open'); }
      });
    });
  ...
  function _respClass(code) { const n = parseInt(code) || 0; if (n >= 200 && n < 300) return 'hau-resp-2xx'; if (n >= 300 && n < 400) return 'hau-resp-3xx'; if (n >= 400) return 'hau-resp-4xx'; return 'hau-resp-5xx'; }
  function _syntaxJSON(json) {
    return json.replace(/("(\\u[a-zA-Z0-9]{4}|\\[^u]|[^\\"])*"(\s*:)?|\b(true|false|null)\b|-?\d+(?:\.\d*)?(?:[eE][+\-]?\d+)?)/g, m => {
      let cls = 'color:#b5d0ff';
      if (/^"/.test(m)) { cls = /:$/.test(m) ? 'color:#9db8ff' : 'color:#a8e6cf'; }
      else if (/true|false/.test(m)) { cls = 'color:#f8a4a4'; }
      else if (/null/.test(m)) { cls = 'color:#888'; }
      return '<span style="' + cls + '">' + m + '</span>';
    });
  }
  function _buildExample(schema, depth) {
    depth = depth || 0; if (depth > 4) return '...'; if (!schema) return null;
    if (schema.$ref) { const n = schema.$ref.split('/').pop(); return _buildExample((spec.components && spec.components.schemas && spec.components.schemas[n]) || {}, depth + 1); }
    if (schema.example !== undefined) return schema.example;
    if (schema.type === 'object' || schema.properties) { const out = {}; Object.entries(schema.properties || {}).forEach(([k, v]) => { out[k] = _buildExample(v, depth + 1); }); return out; }
    if (schema.type === 'array') return [_buildExample(schema.items || {}, depth + 1)];
    return { string: 'string', integer: 0, number: 0.0, boolean: true }[schema.type] ?? null;
  }

  // ── PUBLIC API ──────────────────────────────────────────────────────────────
  global.HikasamiApiUI = { init };
